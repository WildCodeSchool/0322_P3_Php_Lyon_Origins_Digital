    ...
    if (videoId && videoId.length && commentValue && commentValue.length) {
        fetch(url, {
            ...
            .then(function (datas) {
                ...
                const CommentHtml = '<div class="comment-list-0 bt-container mb-4"><div class="row"><div class="fs-3 col-auto text-info d-flex align-items-center border border-end-0 border-1 border-info rounded-start-3 position-relative"><span class="position-absolute top-0 start-100 translate-middle"><i class="bi bi-dot m-0"></i></span><span class="position-absolute top-100 start-100 translate-middle"><i class="bi bi-dot m-0"></i></span></div><div class="col"><div class="d-flex align-items-center justify-content-start"><span class="title-font text-secondary fs-4"><i class="bi bi-dot m-0"></i></span><span class="title-font text-secondary fs-4 me-3">' + datasArray.user + '</span><span class="text-font text-info align-middle"><i class="bi bi-dot m-0"></i>' + datasArray.date + '</span></div><div class="text-font row"><div class="col d-flex align-items-center"><span class="mb-3">'+ datasArray.comment.slice(1, -1); + '</span></div></div></div></div></div>';
